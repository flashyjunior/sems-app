...
export async function GET(request: NextRequest) {
  try {
    ...
    const severity = (url.searchParams.get('severity') as 'high' | 'critical' | 'both') || 'both';
    ...
    const data = await getHighRiskAlerts(start, end, pharmacyId, severity);
    return new Response(JSON.stringify({ success: true, data }), { status: 200 });
  } catch (err) {
    console.error('analytics/dispensing/risk-alerts error', err);
