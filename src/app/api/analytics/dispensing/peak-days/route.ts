...
import { getPeakDays } from '@/services/analytics/aggregationEngine';
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await getPeakDays(start, end, pharmacyId);
    return new Response(JSON.stringify({ success: true, data }), { status: 200 });
  } catch (err) {
    console.error('analytics/dispensing/peak-days error', err);
