...
import { AlertCircle, CheckCircle, XCircle, Loader, ArrowLeft } from 'lucide-react';
...
interface PendingDrugsManagerProps {
  onBack: () => void;
}

export function PendingDrugsManager({ onBack }: PendingDrugsManagerProps) {
  ...
  const getUserName = async (userId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const userName = data.user?.fullName || data.user?.email || data.fullName || userId;
  ...
  useEffect(() => {
    // Load user names for all creators
    const loadUserNames = async () => {
      for (const drug of pendingDrugs) {
        await getUserName(drug.createdByPharmacistId);
      }
    };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-600 mt-1">Review and approve drugs edited by pharmacists</p>
        </div>
      </div>
              ...
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="text-right ml-4">
                  ...
                  <p className="text-sm font-medium text-gray-700">
                    {userNames[drug.createdByPharmacistId] || drug.createdByPharmacistId}
                  </p>
                  <p className="text-xs text-gray-500 mt-2">{formatDate(drug.createdAt)}</p>
                </div>
              </div>
